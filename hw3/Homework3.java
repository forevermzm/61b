...
public class Homework3 {
  ...
  public static void smoosh(int[] ints) {
    ...
    for (int i = 0 ; i < ints.length ; i ++){
      ...
      boolean finishedCheckingDuplicate = false;
      for (int j = i + 1 ; j < ints.length - totalDuplicates; j ++){
        if (!finishedCheckingDuplicate){
          if ( ints [i] == ints[j] ){
            numberOfDuplicates ++;
          } else {
            finishedCheckingDuplicate = true;
          }
        }
        if (finishedCheckingDuplicate) {
          ints[j - numberOfDuplicates] = ints [j];
        }
      }
      totalDuplicates += numberOfDuplicates;
    }

    while (totalDuplicates > 0){
      ints[ints.length - totalDuplicates--] = -1;
    }
