...
public class BinaryTree implements Dictionary {
    ...
    private BinaryTreeNode findHelper(Comparable key, BinaryTreeNode node) {
        ...
        int compareResult = key.compareTo(node.entry.key());
    ...
    public Entry remove(Object key) {
        ...
        if (node == null) return null;
            ...
            if (node.parent == null) {
                if (node.leftChild == null) root = node.rightChild;
                else if (node.rightChild == null) root = node.leftChild;
            } else {
                if (node.leftChild == null) {
            ...
            else if (node.rightChild != null) node.rightChild.parent = node.parent;
            size --;
            return node.entry;
        } else {
            ...
            if (smallestNode != smallestNode.parent.rightChild)
                smallestNode.parent.leftChild = smallestNode.rightChild;
            else 
                smallestNode.parent.rightChild = smallestNode.rightChild;
            if (smallestNode.rightChild != null) smallestNode.rightChild.parent = smallestNode.parent;

            Entry result = node.entry;
            node.entry = smallestNode.entry;
            size --;
            return result;
        }
    }
